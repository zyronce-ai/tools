...
export default function BannerMaker() {
  ...
  const generate = async () => {
    if (!productName.trim() || !offerText.trim()) { toast({ title: "Enter both product name and offer text", variant: "destructive" }); return; }
    ...
    try {
      ...
      } else { throw new Error("Image not generated, try again"); }
  ...
  return (
    <main>
      <SEO title="Banner Maker" description="Create professional banners for your online store" path="/banner" />
      <div className="space-y-6 max-w-4xl mx-auto">
      <AnimatePresence>
        {loading && <ToolLoadingOverlay message="Creating your banner with AI…" />}
      </AnimatePresence>
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2"><Image className="h-6 w-6 text-primary" />{"Promotional Banner Maker"}</h1>
        <p className="text-muted-foreground mt-1">{"Generate promotional banners for your products with AI"}</p>
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader><CardTitle className="text-lg">{"Banner Details"}</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div><Label>{"Product Name *"}</Label><Input placeholder="e.g. Cotton Kurti Set, Bluetooth Earbuds" value={productName} onChange={(e) => setProductName(e.target.value)} /></div>
            <div><Label>{"Offer / Text *"}</Label><Input placeholder="e.g. Flat 50% OFF, Buy 2 Get 1 Free, ₹299 Only" value={offerText} onChange={(e) => setOfferText(e.target.value)} /></div>
            <div>
            ...
            <Button onClick={generate} disabled={loading} className="w-full" size="lg">
              ...
              {loading ? "Generating Banner..." : "Generate Banner"}
            </Button>
          </CardContent>
        </Card>
        ...
        <div>
          {loading && (
            <Card className="flex items-center justify-center h-64">
              <div className="text-center space-y-3"><Loader2 className="h-10 w-10 animate-spin mx-auto text-primary" /><p className="text-muted-foreground">{"AI is creating your banner..."}</p></div>
            </Card>
          )}
          {bannerUrl && !loading && (
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-lg">{"Generated Banner"}</CardTitle>
                <Button variant="outline" size="sm" onClick={downloadBanner}><Download className="h-4 w-4 mr-1" /> {"Download"}</Button>
              </CardHeader>
              <CardContent>
                <img src={bannerUrl} alt="Generated promotional banner" className="w-full rounded-lg border" />
                <p className="text-xs text-muted-foreground mt-3">{"💡 Tip: Don't like it? Generate again for a new design every time!"}</p>
              </CardContent>
            </Card>
          )}
          {!bannerUrl && !loading && (
            <Card className="flex items-center justify-center h-64">
              <div className="text-center text-muted-foreground space-y-2"><Image className="h-12 w-12 mx-auto opacity-30" /><p>{"Banner will appear here"}</p></div>
            </Card>
          )}
        </div>
      </div>
